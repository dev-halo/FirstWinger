...
public class TableRecordParser<TMarshalStruct>
{
    public TMarshalStruct ParseRecordLine(string line)
    {
        ...
        for (int i = 0; i < fieldInfos.Length; ++i)
        {
            ...
            // fieldByte 의 값을 structBytes 에 누적.
    ...
    /// <summary>
    /// 문자열 splited 를 주어진 dataType 에 맞게 fieldByte 배ㅕㄹ에 변환해서 반환.
    /// </summary>
    /// <param name="fieldByte">결과 값을받을 배열.</param>
    /// <param name="dataType">splited 를 변환할 때 사용될 자료형.</param>
    /// <param name="splited">변환 할 값이 있는 문자열.</param>
    protected void MakeBytesByFieldType(out byte[] fieldByte, Type dataType, string splited)
    {
        fieldByte = new byte[1];

        if (typeof(int) == dataType)
        {
            fieldByte = BitConverter.GetBytes(int.Parse(splited));
        }
        else if (typeof(float) == dataType)
        {
            fieldByte = BitConverter.GetBytes(float.Parse(splited));
        }
        else if (typeof(bool) == dataType)
        {
            bool value = bool.Parse(splited);
            int temp = value ? 1 : 0;
        ...
        {
            fieldByte = new byte[MarshalTableConstant.charBufferSize]; // 마샬링을 하기 위해서 고정크기 버퍼를 생성.
            byte[] byteArr = Encoding.UTF8.GetBytes(splited);
