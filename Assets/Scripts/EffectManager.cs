...
public class EffectManager : MonoBehaviour
{
    ...
    private void Prepare()
    {
        for (int i = 0; i < effectFiles.Length; ++i)
        {
            ...
            SystemManager.Instance.GetCurrentSceneMain<InGameSceneMain>().EffectCacheSystem.GenerateCache(effectFiles[i].filePath, go, effectFiles[i].cacheCount, transform);
        }
    }
}
